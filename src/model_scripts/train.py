...
def train(config):
    ...
    X_train,y_train = df_train.drop(columns = ['Daily_Revenue']).values, df_train['Daily_Revenue'].values
    X_val, y_val = df_test.drop(columns = ['Daily_Revenue']).values, df_test['Daily_Revenue'].values
    ...
    with mlflow.start_run():
        ...
        model_dir = os.path.dirname(config['train']['model_path'])
        os.makedirs(model_dir, exist_ok=True)

        with open(config['train']['model_path'], "wb") as file:
            pickle.dump(best, file)

        power_dir = os.path.dirname(config['train']['power_path'])
